...
const PROVEEDORES = {
    ...
    "NTTDATA": {
        "id_proveedor": "44",
        "reportes": ['PuntualidadYReaperturas', 'PuntualidadYReaperturasPadTot', 'PuntualidadYReaperturasPadTotODS', 'Resumen_Pendientesxnivel', 'Resumen_PendientesxTotalPad', 'Resumen_PendientesxTotalPadODS', 'Ingresos_N1', 'alegaciones']
    }
};
...
async function getMesesYaDescargados(providerName) {
    ...
    try {
        ...
        if (error.code === '42P01') { 
            ...
            return []; // Asegura devolver un array
        }
        console.error('❌ Error consultando meses en PostgreSQL:', error);
        return []; // Asegura devolver un array en caso de otros errores
    } finally {
        if (client) {
            client.release();
        }
...
async function uploadToPostgres(dataToUpload) {
    ...
    const MASTER_COLUMNS = [
        "Proveedor", "MesConsulta", "TipoReporte", "PROV", "ORGANIZACION", "NIVEL", "SLA", "HORARIO",
        "MES", "ANIO", "NUM_INCIDENTE", "FECHA_INI", "FECHA_FIN", "GRUPO", "RUTA", "PRIORIDAD",
        "ESTADO", "SUBESTADO", "FECHA_REAPERTURA", "IMPACTO", "MINUTOSHAB", "STATUS_ACTUAL",
        "FECHACREACION", "APERTURA", "CIERRE", "IDPROV"
    ];
    ...
    const normalizedData = dataToUpload.map(originalRow => {
        ...
        for (const col of MASTER_COLUMNS) {
            ...
            if (TIMESTAMP_COLUMNS.includes(col) && value === '') {
                value = null;
            }
    ...
    try {
        ...
        console.error("❌ Error al enviar datos a PostgreSQL:", error);
    } finally {
        client.release();
    }
}
...
async function ingestAllData() {
    ...
    if (allReportsData.length > 0) {
        ...
        const dataConIncidente = allReportsData.filter(record => {
            return record.NUM_INCIDENTE && String(record.NUM_INCIDENTE).trim() !== '';
        });
...
// Llamada para que el script se pueda ejecutar directamente para pruebas
ingestAllData();
